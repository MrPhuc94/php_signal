<form method="post" action="javascript:void(0)" id="updatewithdrawalinfo">
    @csrf
    @method('PUT')
    @if ($bankTransfer && $bankTransfer->status == 'enabled')
        <fieldset>
            <div class="form-row">
    ...
    <fieldset class="mt-2">
        <div class="form-row">
            @if ($bitcoin && $bitcoin->status == 'enabled')
                <div class="form-group col-md-6">
            ...
            @endif
            @if ($ethereum && $ethereum->status == 'enabled')
                <div class="form-group col-md-6">
            ...
            @endif
            @if ($litecoin && $litecoin->status == 'enabled')
                <div class="form-group col-md-6">
    ...
    @if (
        ($usdt && $usdt->status == 'enabled') ||
            ($litecoin && $litecoin->status == 'enabled') ||
            ($ethereum && $ethereum->status == 'enabled') ||
            ($bitcoin && $bitcoin->status == 'enabled') ||
            ($bankTransfer && $bankTransfer->status == 'enabled'))
