...
<html>
  ...
  function importDatabase($mysql_host, $mysql_database, $mysql_user, $mysql_password, $admin_email, $admin_password,$license,$app_url){
    ...
    try {
      ...
      $stmt = $db->prepare("INSERT INTO admins (firstName, lastName, email, password, acnt_type_active, status, type, created_at, updated_at) VALUES (:firstName, :lastName, :email, :password, :acnt_type_active, :status, :type, :created_at, :updated_at)");
      $stmt->bindParam(':firstName', $firstName);
      $stmt->bindParam(':lastName', $lastName);
      $stmt->bindParam(':email', $admin_email);
      $stmt->bindParam(':password', $hashedPassword);
      $stmt->bindParam(':acnt_type_active', $acnt_type_active);
      ...
      $status = "active";
      $type = "Super Admin";
      $created_at = "2024-03-04 14:51:06";
      $updated_at = "2024-03-04 14:51:06";
      $stmt->execute();
      
      //update purchase code 
      
      $sql = "UPDATE settings_conts SET purchase_code='$license' WHERE id=1";
      ...
      $sql = "UPDATE settings SET site_address='$app_url' WHERE id=1";
  ...
  if(importDatabase($db_host,$db_name,$db_user,$db_pass,$admin_email,$admin_password,$license,$app_url)){
  ...
  <?php
    if ($_POST['license']) {
      ...
      if($vrfy_lic->error == true){
        ...
        <a class='button is-warning' href='?action=config&lic=$license'>Next Step <i class='fa fa-angle-double-right'></i></a>
        ";
      }
...
<br>
  ...
  <input class="form-control input-lg" name="admin_email" placeholder="Enter your email" type="text" required=""><br>
  <input class="form-control input-lg" name="admin_password" placeholder="Enter password" type="text" required=""><br>
