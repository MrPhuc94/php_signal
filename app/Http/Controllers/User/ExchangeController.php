...
class ExchangeController extends Controller
{
    ...
    public function assetview()
    {
        ...
        $assets = CryptoCurrency::latest()->get();
    ...
    public function history()
    {
        ...
        return view("{$settings->theme}.user.crypto-transaction", [
            ...
            'transactions' => DB::table('crypto_records')->where('user_id', auth()->user()->id)->orderByDesc('id')->paginate(10),
        ]);
    }
    ...
    public function getprice($base, $quote, $amount)
    {
        ...
        $qCurr = CryptoCurrency::where('symbol', strtoupper($quote))->first();
        $bCurr = CryptoCurrency::where('symbol', strtoupper($base))->first();
        ...
        if ($quote == "usd") {
            if ($manualb) {
                $dollar = floatval($bCurr->price_in_usd);
            } else {
                $dollar = $this->get_rate($base, 'usd');
            }
            $mainbal = $amout_to * $dollar;
            $prices = round($mainbal, 8);
        } elseif ($base == "usd" && $quote != 'usdt') {
            if ($manualq) {
                $dollar = floatval($qCurr->price_in_usd);
            } else {
            ...
            if ($manualq) {
                $rate2 = floatval($qCurr->price_in_usd);
            } else {
                $rate2 =  $this->get_rate($quote, 'usd', "price");
            }
            if ($manualb) {
                $rate1 =  floatval($bCurr->price_in_usd);
            } else {
    ...
    public function exchange(Request $request)
    {
        ...
        if ($request->source == 'usd') {
            ...
            $manual = DB::table('crypto_currencies')->where('symbol', strtoupper($request->destination))->exists();
            if ($manual) {
                $balToUpdate = "{$request->destination}_balance";
                DB::table('crypto_accounts')
                    ->where('user_id', $user->id)
                    ->update([
                        $balToUpdate => $cryptobalances->$balToUpdate + $request->quantity,
                    ]);
            }
        ...
        if ($request->source != 'usd' and  $request->destination != 'usd') {
            ...
            $manual = DB::table('crypto_currencies')->where('symbol', strtoupper($request->source))->exists();
            if ($manual) {
                $balToUpdate = "{$request->destination}_balance";
                $source = $cryptobalances->$balToUpdate;
            } else {
                $source = $cryptobalances->$src;
                $balToUpdate = $src;
            }

            if (DB::table('crypto_currencies')->where('symbol', strtoupper($request->destination))->exists()) {
                $column = "{$request->destination}_balance";
                $destination = $column;
                $dest = $cryptobalances->$column;
            } else {
                $destination = $request->destination;
                $dest = $cryptobalances->$cdest;
            }

            if ($source < $request->amount) {
                return response()->json(['status' => 201, 'message' => 'Insuficient fund in your source account']);
            }

            CryptoAccount::where('user_id', $user->id)
                ->update([
                    $balToUpdate => $source - $request->amount,
                ]);

            CryptoAccount::where('user_id', $user->id)
                ->update([
                    $destination => $dest + $request->quantity,
                ]);
        ...
        if ($request->source != 'usd' and  $request->destination == 'usd') {
            ...
            if ($manual) {
                $balToUpdate = "{$request->source}_balance";
                $source = $cryptobalances->$balToUpdate;
            } else {
                $source = $cryptobalances->$src;
                $balToUpdate = $src;
