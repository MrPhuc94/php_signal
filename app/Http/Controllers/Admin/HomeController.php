...
class HomeController extends Controller
{
    ...
    public function addCrypto(Request $request): RedirectResponse
    {
        if ($request->has('assetId')) {
            CryptoCurrency::where('id', $request->assetId)->update($request->except(['assetId', '_token']));
            return redirect()->back()->with('success', 'Token Updated Successfuly');
        } else {
            CryptoCurrency::create($request->except('_token'));
            Schema::table('crypto_accounts', function (Blueprint $table) use ($request) {
                $table->string(strtolower($request->symbol) . '_balance', 12)->default(0.00)->after('user_id');
            });

            return redirect()->back()->with('success', 'Token Added Successfuly');
        }
    }
    ...
    {
        return  view('admin.Settings.email_templates.show', [
