...
class AppSettingsController extends Controller
{
    ...
    public function saveTemplate(Request $request): RedirectResponse
    {
        ...
        if ($request->type == 'membership') {
            $settings->membership_email_template = $request->membership;
        }

        if ($request->type == 'funded') {
            $settings->funded_email_template = $request->funded;
        }

        if ($request->type == 'direct') {
            $settings->direct_trade_email_template = $request->direct;
        }
